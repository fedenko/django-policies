...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-policies runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-policies.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_policies")
...
def lint(c):
    ...
    """
    c.run("flake8 django-policies tests")
...
def release(c, bumpsize=""):
    ...
    c.run(
        'git tag -a {version} -m "New version: {version}"'.format(
            version=django_policies.__version__
        )
    )
    c.run("git push --tags")
